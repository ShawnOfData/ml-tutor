...
from ml_tutor.services.memory import paths
from ml_tutor.services.memory import snapshot as snap
from ml_tutor.services.memory.consolidator.chunker import (
    chunk_with_boundary,
)
from ml_tutor.services.memory.consolidator.meta import (
    load_l2_meta,
    load_l3_meta,
    save_l2_meta,
    save_l3_meta,
)
from ml_tutor.services.memory.consolidator.modes._runtime import (
    OnEvent,
    call_llm,
    emit,
    load_doc,
...
from ml_tutor.services.memory.consolidator.references import (
    ExtractedFact,
    refs_in_span_l2,
    refs_in_span_l3,
...
from ml_tutor.services.memory.document import Document, Entry, serialize
from ml_tutor.services.memory.ops import AddOp
from ml_tutor.services.memory.ops import apply as apply_ops
from ml_tutor.services.memory.paths import L3Slot, Surface
from ml_tutor.services.memory.settings import load_memory_settings
    ...
    *,
    ...
    if not new_entities:
        ...
        if settings.merge.auto_after_update:
            from ml_tutor.services.memory.consolidator.modes.merge import run_merge
    ...
    if settings.dedup.auto_after_update and facts_added > 0:
        ...
        from ml_tutor.services.memory.consolidator.modes.dedup import run_dedup
...
def _load_all_l2_docs() -> dict[str, Document]:
    from ml_tutor.services.memory.document import parse
