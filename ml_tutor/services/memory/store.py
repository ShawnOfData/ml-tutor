...
from ml_tutor.services.memory import consolidator, paths, trace
from ml_tutor.services.memory.consolidator import ConsolidateResult, OnEvent
from ml_tutor.services.memory.document import Document, parse, serialize
from ml_tutor.services.memory.ops import AddOp, ApplyReport, EditOp
from ml_tutor.services.memory.ops import apply as ops_apply
...
_NO_MEMORY = (
    "(No memory available — interact with ML Tutor and update from the Memory page to build one.)"
)
...
class MemoryStore:
    ...
    async def apply_ops_payload(
        ...
        """
        from ml_tutor.services.memory.consolidator import _parse_ops_response
