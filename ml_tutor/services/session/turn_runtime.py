...
from ml_tutor.core.stream import StreamEvent, StreamEventType
from ml_tutor.services.llm.utils import clean_thinking_tags
from ml_tutor.services.path_service import get_path_service
from ml_tutor.services.session.protocol import SessionStoreProtocol

if TYPE_CHECKING:
    from ml_tutor.services.llm.config import LLMConfig
...
def _llm_selection_dict(value: Any) -> dict[str, str] | None:
    from ml_tutor.services.model_selection import LLMSelection
...
class TurnRuntimeManager:
    ...
    def __init__(self, store: SessionStoreProtocol | None = None) -> None:
        from ml_tutor.services.session import get_session_store
    ...
    async def start_turn(self, payload: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
        ...
        try:
            from ml_tutor.capabilities.request_contracts import validate_capability_config
        ...
        if llm_selection:
            try:
                from ml_tutor.multi_user.model_access import apply_allowed_llm_selection
            ...
            from ml_tutor.multi_user.model_access import redacted_model_access
        ...
        if llm_selection:
            from ml_tutor.services.config import get_model_catalog_service
            from ml_tutor.services.model_selection import (
                LLMSelection,
        ...
        if payload.get("tools") is None:
            try:
                from ml_tutor.api.routers.settings import get_enabled_optional_tools
    ...
    async def _run_turn(self, execution: _TurnExecution) -> None:
        ...
        try:
            from ml_tutor.agents.notebook import NotebookAnalysisAgent
            from ml_tutor.book.context import build_book_context
            from ml_tutor.core.context import Attachment, UnifiedContext
            from ml_tutor.runtime.orchestrator import ChatOrchestrator
            from ml_tutor.services.memory import get_memory_store
            from ml_tutor.services.model_selection.runtime import (
                activate_llm_selection,
            )
            from ml_tutor.services.model_selection.runtime import (
                reset_llm_selection as reset_active_llm_selection,
            )
            from ml_tutor.services.notebook import get_notebook_manager
            from ml_tutor.services.session.context_builder import ContextBuilder
            from ml_tutor.services.skill import get_skill_service
            ...
            from ml_tutor.services.storage import get_attachment_store
            ...
            from ml_tutor.utils.document_extractor import extract_documents_from_records
            ...
            from ml_tutor.multi_user.context import get_current_user
            from ml_tutor.multi_user.paths import get_admin_path_service
            from ml_tutor.multi_user.skill_access import assigned_skill_ids
            from ml_tutor.services.skill.service import SkillService
            ...
            if is_chat_capability:
                from ml_tutor.services.session.source_inventory import (
                    build_inventory,
                    render_manifest,
                )
    ...
    async def _maybe_generate_session_title(
        ...
        *,
        ...
        title = ""
        try:
            from ml_tutor.services.llm import stream as llm_stream
