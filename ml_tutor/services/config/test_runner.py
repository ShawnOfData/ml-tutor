...
class ConfigTestRunner:
    ...
    async def _test_llm(self, run: TestRun, catalog: dict[str, Any]) -> None:
        from ml_tutor.services.llm import clear_llm_config_cache, get_token_limit_kwargs
        from ml_tutor.services.llm import complete as llm_complete
    ...
    async def _test_embedding(
        ...
        from ml_tutor.services.embedding.client import EmbeddingClient
        from ml_tutor.services.embedding.config import EmbeddingConfig
        ...
        probe_texts = [
            "ML Tutor embedding smoke test",
            "ML Tutor retrieval batch probe",
        ]
    ...
    def _test_search(self, run: TestRun, catalog: dict[str, Any]) -> None:
        from ml_tutor.services.search import web_search
        ...
        run.emit("info", "Running search query: ML Tutor configuration health check")
        result = web_search("ML Tutor configuration health check", provider=provider)
        run.emit(
            "response",
