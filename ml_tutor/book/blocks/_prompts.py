"""
Shared prompt loader for non-BaseAgent call sites in ``ml_tutor/book``.

All LLM-facing prompts inside this module live as YAML under
``ml_tutor/book/prompts/{en,zh}/<name>.yaml`` and are loaded through the
unified :class:`~ml_tutor.services.prompt.PromptManager`. This helper is the
...
def load_book_prompts(name: str, language: str) -> dict[str, Any]:
    ...
    Args:
        name: File stem under ``ml_tutor/book/prompts/{lang}/``
    ...
    if not prompts:
        raise RuntimeError(
            ...
            f"Expected ml_tutor/book/prompts/{{en,zh}}/{name}.yaml."
        )
    return prompts
