...
Reuses:
- ``ml_tutor.services.session.get_sqlite_session_store`` for chat history
- ``ml_tutor.services.notebook.notebook_manager`` for notebook records
- ``ml_tutor.agents.notebook.NotebookAnalysisAgent`` for context distillation
"""
...
async def _resolve_chat_selections(
    ...
    *,
    ...
    if not selections:
        return []
    try:
        from ml_tutor.services.session import get_sqlite_session_store
...
async def _resolve_notebook_context(
    ...
    *,
    ...
    if not notebook_refs:
        return "", 0
    try:
        from ml_tutor.services.notebook import notebook_manager
