...
from ml_tutor.core.tool_protocol import BaseTool, ToolDefinition, ToolParameter, ToolResult
from ml_tutor.tools.prompting import load_prompt_hints
...
class BrainstormTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from ml_tutor.tools.brainstorm import brainstorm
...
class RAGTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from ml_tutor.tools.rag_tool import rag_search
...
class WebSearchTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from ml_tutor.tools.web_search import web_search
...
"""
    ...
    async def _generate_code(self, intent: str) -> str:
        from ml_tutor.services.llm import complete, get_token_limit_kwargs
...
class ReasonTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from ml_tutor.tools.reason import reason
...
class PaperSearchToolWrapper(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from ml_tutor.tools.paper_search_tool import ArxivSearchTool
...
class GeoGebraAnalysisTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from ml_tutor.agents.vision_solver.vision_solver_agent import VisionSolverAgent
        from ml_tutor.services.llm.config import get_llm_config
...
class WebFetchTool(_PromptHintsMixin, BaseTool):
    ...
    ``ml_tutor.tools.web_fetch`` so this wrapper stays free of network
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from ml_tutor.tools.web_fetch import (
            DEFAULT_MAX_CHARS,
...
class ListNotebookTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from ml_tutor.tools.list_notebook import list_notebooks_or_records
...
class WriteNoteTool(_PromptHintsMixin, BaseTool):
    ...
    * ``edit`` — patch an existing record's title / body / summary.
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from ml_tutor.tools.write_note import write_note
...
class GithubTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from ml_tutor.tools.github_query import run_github_query
...
class AskUserTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from ml_tutor.tools.ask_user import build_ask_user_payload
