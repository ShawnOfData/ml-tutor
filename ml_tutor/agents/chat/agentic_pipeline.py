...
lives in :mod:`ml_tutor.core.agentic`: label parsing, single-call streaming,
...
from ml_tutor.agents._shared.tool_composition import (
    ToolMountFlags,
...
from ml_tutor.core.agentic.labels import find_inline_labels, strip_label_probe_prefix
from ml_tutor.core.agentic.tool_dispatch import MAX_PARALLEL_TOOL_CALLS
from ml_tutor.core.context import UnifiedContext
from ml_tutor.core.stream_bus import StreamBus
from ml_tutor.core.trace import (
    build_trace_metadata,
    derive_trace_metadata,
    merge_trace_metadata,
    new_call_id,
)
from ml_tutor.runtime.registry.tool_registry import get_tool_registry
from ml_tutor.services.config import get_chat_params, load_system_settings  # noqa: F401
from ml_tutor.services.llm import (
    clean_thinking_tags,
    get_llm_config,
...
from ml_tutor.services.llm.context_window import resolve_effective_context_window
from ml_tutor.services.prompt import get_prompt_manager
from ml_tutor.services.prompt.language import append_language_directive
from ml_tutor.services.provider_registry import find_by_name
...
def _classify_answer_now_label(
    ...
    *,
    ...
    """
    from ml_tutor.core.agentic.labels import classify_label
...
class AgenticChatPipeline:
    ...
    async def run(self, context: UnifiedContext, stream: StreamBus) -> None:
        ...
        # ``currentStage`` indicator ("ML Tutor responding…"). It carries no
    ...
    async def _execute_tool_call(
        ...
        *,
        ...
        """
        from ml_tutor.core.agentic import execute_tool_call
    ...
    def _estimate_messages_tokens(messages: list[dict[str, Any]]) -> int:
        ...
        from ml_tutor.services.session.context_builder import count_tokens
    ...
    def _build_system_prompt(
        ...
        # YAML under ``ml_tutor/tools/prompting/hints/{lang}/{tool}.yaml``.
    ...
    def _build_messages(
        ...
        *,
        ...
        model_context = (context.config_overrides or {}).get("model_context", "").strip()
        if model_context:
            system_parts.append(
                f"当前用户在学习模型「{model_context}」的相关内容，请针对该模型的具体原理、公式、代码和应用场景进行回答。"
            )
        messages: list[dict[str, Any]] = [
            {"role": "system", "content": "\n\n---\n\n".join(system_parts)}
        ]
    ...
    def _extract_answer_now_context(context: UnifiedContext) -> dict[str, Any] | None:
        from ml_tutor.capabilities._answer_now import extract_answer_now_context
...
class _ChatLoopHost:
    ...
    :class:`~ml_tutor.core.agentic.LoopHost` protocol calls to them.
    """
