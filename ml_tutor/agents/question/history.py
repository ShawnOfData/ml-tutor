...
* :func:`load_session_quiz_history` — async, takes ``session_id`` and an
  ...
  :class:`~ml_tutor.agents.question.pipeline.QuizHistoryEntry`.
...
from ml_tutor.agents.question.pipeline import QuizHistoryEntry
...
async def load_session_quiz_history(
    ...
    *,
    ...
    if not session_id or max_entries <= 0:
        return []
    try:
        from ml_tutor.services.session.sqlite_store import get_sqlite_session_store
