...
class DeepQuestionCapability(BaseCapability):
    manifest = CapabilityManifest(
        ...
        trigger_keywords=["出题", "生成题目", "练习题", "quiz", "deep question", "deep_question"],
    )
    ...
    async def run(self, context: UnifiedContext, stream: StreamBus) -> None:
        ...
        if isinstance(followup_question_context, dict) and followup_question_context.get(
            ...
            from ml_tutor.agents.question.agents.followup_agent import FollowupAgent
        ...
        if mode != "mimic":
            ...
            from ml_tutor.agents.question.history import load_session_quiz_history
            from ml_tutor.agents.question.pipeline import QuestionPipeline
            from ml_tutor.agents.question.request_config import (
    ...
    async def _run_mimic_mode(
        ...
        * ``[Attached Documents]`` in  → no paper available; fall back to
        ...
        """
        from ml_tutor.agents.question.history import load_session_quiz_history
        from ml_tutor.agents.question.mimic_source import (
