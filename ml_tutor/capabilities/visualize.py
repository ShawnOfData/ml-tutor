...
from ml_tutor.capabilities.request_contracts import (
...
from ml_tutor.core.trace import merge_trace_metadata
...
class VisualizeCapability(BaseCapability):
    manifest = CapabilityManifest(
        ...
        trigger_keywords=["可视化", "画图", "绘图", "图表", "visualize", "visualization", "diagram"],
    )

    async def run(self, context: UnifiedContext, stream: StreamBus) -> None:
        from ml_tutor.agents.visualize.models import ReviewResult
        from ml_tutor.agents.visualize.pipeline import VisualizePipeline
        from ml_tutor.agents.visualize.utils import (
            build_fallback_html,
        ...
        *,
        ...
        if importlib.util.find_spec("manim") is None:
            raise RuntimeError(
                ...
                "Install with `pip install 'ml_tutor[math-animator]'` "
                "or `pip install -r requirements/math-animator.txt`."
            )

        from ml_tutor.agents.math_animator.pipeline import MathAnimatorPipeline
        from ml_tutor.agents.math_animator.request_config import MathAnimatorRequestConfig
        from ml_tutor.core.trace import build_trace_metadata, new_call_id
        from ml_tutor.services.llm.config import get_llm_config
    ...
    async def _run_answer_now(
        ...
        *,
        ...
        from ml_tutor.capabilities._answer_now import (
