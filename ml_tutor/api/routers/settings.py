...
from ml_tutor.multi_user.model_access import allowed_llm_options, redacted_model_access
...
from ml_tutor.services.embedding.client import reset_embedding_client
from ml_tutor.services.llm.client import reset_llm_client
from ml_tutor.services.llm.config import clear_llm_config_cache
from ml_tutor.services.model_selection import list_llm_options
from ml_tutor.services.path_service import get_path_service
from ml_tutor.tools.builtin import USER_TOGGLEABLE_TOOL_NAMES
...
def _profile_file() -> Path:
    return get_path_service().get_settings_file("profile")
...
def _provider_choices() -> dict[str, list[dict[str, str]]]:
    ...
    from ml_tutor.services.config.provider_runtime import EMBEDDING_PROVIDERS
    from ml_tutor.services.provider_registry import PROVIDERS
...
async def fetch_models_from_provider(payload: FetchModelsPayload):
    ...
    """
    _require_settings_admin()
    from ml_tutor.services.llm.factory import fetch_models as fetch_llm_models
...
@router.put("/profile")
async def update_profile(payload: SurveyProfile):
    current = load_profile()
    current["survey"] = payload.model_dump()
    save_profile(current)
    return {"survey": payload.model_dump()}
...
async def complete_tour(payload: TourCompletePayload | None = None):
    ...
    return {
        "status": "completed",
        "message": "Configuration saved. ML Tutor will restart shortly.",
        "launch_at": launch_at,
