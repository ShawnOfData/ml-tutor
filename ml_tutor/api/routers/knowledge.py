...
from ml_tutor.api.utils.progress_broadcaster import ProgressBroadcaster
from ml_tutor.api.utils.task_id_manager import TaskIDManager
from ml_tutor.api.utils.task_log_stream import capture_task_logs, get_task_stream_manager
from ml_tutor.knowledge.add_documents import DocumentAdder
from ml_tutor.knowledge.initializer import KnowledgeBaseInitializer
from ml_tutor.knowledge.manager import KnowledgeBaseManager
from ml_tutor.knowledge.naming import validate_knowledge_base_name
from ml_tutor.knowledge.progress_tracker import ProgressStage, ProgressTracker
from ml_tutor.multi_user.context import get_current_user
from ml_tutor.multi_user.knowledge_access import (
    assert_writable,
    current_kb_base_dir,
    current_kb_manager,
...
from ml_tutor.services.config import PROJECT_ROOT, load_config_with_main
from ml_tutor.services.rag.factory import DEFAULT_PROVIDER
from ml_tutor.services.rag.file_routing import FileTypeRouter
from ml_tutor.utils.document_validator import DocumentValidator
from ml_tutor.utils.error_utils import format_exception_message
...
def _save_zip_archive(
    ...
    from ml_tutor.utils.archive_extractor import ArchiveTooLargeError, safe_extract_zip
...
def _save_uploaded_files(
    ...
    from ml_tutor.services.pocketbase_client import is_pocketbase_enabled
...
def _upload_file_to_pb(kb_name: str, filename: str, file_path: Path) -> None:
    ...
    try:
        from ml_tutor.services.pocketbase_client import get_pb_client
...
def _matching_index_is_valid(kb_name: str, matching_version: dict | None) -> bool:
    ...
    if not matching_version:
        return False
    try:
        from ml_tutor.services.rag.pipelines.llamaindex.storage import (
...
async def get_all_kb_configs():
    ...
    try:
        from ml_tutor.services.config import get_kb_config_service
...
async def reindex_knowledge_base(
    ...
    try:
        ...
        from ml_tutor.services.rag.embedding_signature import signature_from_embedding_config
...
async def websocket_progress(websocket: WebSocket, kb_name: str):
    ...
    from ml_tutor.api.routers.auth import ws_auth_failed, ws_require_auth
    from ml_tutor.multi_user.context import reset_current_user
    ...
    try:
        ...
        from ml_tutor.services.rag.index_versioning import list_kb_versions
