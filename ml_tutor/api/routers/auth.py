...
from ml_tutor.services.config import load_auth_settings
...
from ml_tutor.multi_user.context import set_current_user, user_from_token_payload
from ml_tutor.multi_user.paths import local_admin_user
from ml_tutor.services.auth import (
    AUTH_ENABLED,
    POCKETBASE_ENABLED,
    TOKEN_EXPIRE_HOURS,
    TokenPayload,
...
async def ws_require_auth(ws: WebSocket) -> _CtxToken | _WsAuthFailed:
    ...
    _auth_enabled = bool(load_auth_settings().get("enabled", False))
...
def _local_admin_token_payload() -> TokenPayload:
    ...
    ``ml_tutor/multi_user/paths.py``.
    """
    from ml_tutor.multi_user.models import LOCAL_ADMIN_ID, LOCAL_ADMIN_USERNAME
