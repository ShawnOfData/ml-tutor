...
def _discover_plugins() -> list[Any]:
    try:
        from ml_tutor.plugins.loader import discover_plugins
    except Exception:
...
async def _execute_stream(tool_name: str, params: dict[str, Any]) -> AsyncGenerator[str, None]:
    ...
    stdout_stream = _QueueTextStream(
        event_queue, loop, stream=None, logger_name="ml_tutor.playground.stdout"
    )
    stderr_stream = _QueueTextStream(
        event_queue, loop, stream=None, logger_name="ml_tutor.playground.stderr"
    )
...
async def _execute_capability_stream(
    ...
    if capability_name == "chat" and bot_id:
        from ml_tutor.api.routers.tutorbot import (
            ChatMessageRequest,
            _bot_chat_stream,
            _ensure_running_bot,
        )
