"""Branded banner + localized labels for ``ml_tutor start`` / ``ml_tutor init``.
...
from ml_tutor.__version__ import __version__
...
LABELS: dict[str, dict[str, str]] = {
    "en": {
        ...
        "init.next_step": "Run `ml_tutor start` to launch ML Tutor.",
        "init.choice": "Choice",
        ...
        "start.frontend_restart_failed": (
            ...
            "Stop PID {pid} and run `ml_tutor start` again."
        ),
        ...
        "start.port_in_use": (
            "ML Tutor cannot start because port(s) already in use: {ports}. "
    ...
    "zh": {
        ...
        "init.next_step": "运行 `ml_tutor start` 启动 ML Tutor。",
        "init.choice": "请选择",
        ...
        "start.frontend_restart_failed": (
            ...
            "请先停止 PID {pid},然后重新运行 `ml_tutor start`。"
        ),
...
def render_banner(language: str | None, *, mode_key: str | None = None) -> Panel:
    ...
    return Panel(
        Align.left(body),
        title="[bold bright_cyan]ML Tutor[/]",
