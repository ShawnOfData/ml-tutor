...
from ml_tutor.core.capability_protocol import BaseCapability
from ml_tutor.runtime.bootstrap.builtin_capabilities import BUILTIN_CAPABILITY_CLASSES
...
def _load_plugin_hooks():
    try:
        module = importlib.import_module("ml_tutor.plugins.loader")
    except Exception:
...
class CapabilityRegistry:
    ...
    def match_by_intent(self, text: str) -> str | None:
        text_lower = text.lower()
        for name, cap in self._capabilities.items():
            for kw in cap.manifest.trigger_keywords:
