"""Local Web launcher for the installed ML Tutor app."""
...
from ml_tutor.runtime.banner import labels_for, print_banner, resolve_language
from ml_tutor.runtime.home import DEEPTUTOR_HOME_ENV, PACKAGE_ROOT, get_runtime_home
...
def _reset_runtime_singletons() -> None:
    ...
    try:
        from ml_tutor.services.path_service import PathService

        PathService.reset_instance()
    except Exception:
        pass
    try:
        from ml_tutor.services.config.runtime_settings import RuntimeSettingsService

        RuntimeSettingsService._instances.clear()
    except Exception:
        pass
    try:
        from ml_tutor.services.config.model_catalog import ModelCatalogService
...
def _packaged_web_dir() -> Path | None:
    try:
        ...
        return None
    path = Path(ml_tutor_web.__file__).resolve().parent
...
def _copy_packaged_web_if_needed(
    ...
    *,
    ...
    marker = cache / ".ml_tutor-web-runtime.json"
...
def _resolve_frontend(
    ...
    *,
    ...
    if packaged is not None:
        if not node:
            raise SystemExit("Node.js 20+ is required to run the packaged ML Tutor Web app.")
    ...
    raise SystemExit(
        "ML Tutor Web assets are not installed. Install the full app with `pip install -U ml_tutor`, "
