...
from ml_tutor.events.event_bus import Event, EventType, get_event_bus
...
class ChatOrchestrator:
    ...
    async def handle(self, context: UnifiedContext) -> AsyncIterator[StreamEvent]:
        ...
        if cap_name == "chat":
            matched = self._cap_registry.match_by_intent(context.user_message or "")
            if matched:
                logger.info("Intent match: '%s' → capability '%s'", context.user_message, matched)
                cap_name = matched
